<script lang="ts">
  ...
  function renderTurnstileWidget() {
    ...
    if (!container || !turnstile) {
      log('Cannot render - container or turnstile not ready', { container: !!container, turnstile: !!turnstile });
      return;
    }
    ...
    if (turnstileWidgetId) {
      try {
        ...
        log('Error removing old widget:', e);
      }
      turnstileWidgetId = null;
    }
    
    log('Rendering Turnstile widget manually');
    turnstileWidgetId = turnstile.render(container, {
      sitekey: TURNSTILE_SITE_KEY,
      callback: (token: string) => {
        log('Turnstile callback received token');
        turnstileToken = token;
      },
      'error-callback': () => {
        log('Turnstile error callback');
      },
      'expired-callback': () => {
    ...
    log('Turnstile widget rendered with ID:', turnstileWidgetId);
  }
  ...
  $effect(() => {
    ...
    if (currentStep === 3 && turnstileLoaded && isOpen) {
      log('On step 4 with Turnstile loaded - attempting manual render');
      
      // Use setTimeout to ensure the DOM element exists after Svelte renders
      setTimeout(() => {
        ...
        {#if currentStep === 3}
          <div class="space-y-4">
            ...
            <div class="mt-4 pt-4 border-t border-gray-100">
              ...
              {#if !turnstileLoaded}
                <div class="flex items-center gap-2 text-sm text-gray-500 min-h-[65px]">
                  <svg class="animate-spin w-4 h-4" fill="none" viewBox="0 0 24 24">
                    <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                    <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                  </svg>
                  Beveiligingswidget laden...
                </div>
              {:else}
